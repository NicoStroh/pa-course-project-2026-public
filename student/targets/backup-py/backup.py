...
class Backup:
    ...
    @staticmethod
    def load_restore_state(state_file: Path) -> Result[None]:
        """Load a saved extraction state from disk."""
        if not state_file.exists():
            return Err(f"Restore state file '{state_file}' does not exist.")

        try:
            with open(state_file, "rb") as fd:
                pickle.load(fd)
        except (IOError, OSError, pickle.PickleError) as err:
            return Err(f"Failed to load restore state: {err}.")
...
def main():
    ...
    parser.add_argument(
        "--restore-state",
        metavar="STATE",
        help="Load a saved extraction state file before resuming an extract operation"
    )
    ...
    if args.backup:
        ...
        if args.restore_state:
            restore_res = Backup.load_restore_state(Path(args.restore_state))
            match restore_res:
                case Err(error=e):
                    print(f"{e}", file=sys.stderr)
                    sys.exit(1)
