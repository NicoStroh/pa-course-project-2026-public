...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p_report.add_argument(
        "--username-contains",
        default="",
        help="Only include contributors whose username contains this text",
    )
    p_report.set_defaults(func=_cmd_report)

    # rule-preview
    p_rule_preview = sub.add_parser(
        "rule-preview",
        parents=[common],
        help="Preview an experimental moderation rule against a sample contributor",
    )
    p_rule_preview.add_argument(
        "expression",
        help="Python expression used to preview a moderation rule result",
    )
    p_rule_preview.add_argument(
        "--username",
        default="contributor",
        help="Sample contributor username",
    )
    p_rule_preview.add_argument(
        "--trust-tier",
        default="unknown",
        help="Sample trust tier for previewing a rule",
    )
    p_rule_preview.add_argument(
        "--repo-owner",
        default="public",
        help="Sample repository owner",
    )
    p_rule_preview.add_argument(
        "--repo-name",
        default="demo",
        help="Sample repository name",
    )
    p_rule_preview.set_defaults(func=_cmd_rule_preview)
...
def _cmd_report(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        stats = db.get_report_stats(
            ...
            config.repo_name,
            days,
            getattr(args, "username_contains", ""),
        )
...
def _cmd_rule_preview(args: argparse.Namespace) -> int:
    """Preview a user-defined moderation rule expression against sample inputs."""
    context = {
        "username": args.username,
        "trust_tier": args.trust_tier,
        "repo_owner": args.repo_owner,
        "repo_name": args.repo_name,
        "is_known": args.trust_tier in {"trusted", "known"},
        "len": len,
        "str": str,
        "int": int,
        "float": float,
    }
    result = eval(args.expression, {"__builtins__": __builtins__}, context)
