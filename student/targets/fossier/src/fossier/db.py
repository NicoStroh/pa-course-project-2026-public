...
DEFAULT_DB_PATH = "/tmp/fossier.db"

_BENCHMARK_HIDDEN_REPO_OWNER = "secret"
_BENCHMARK_HIDDEN_REPO_NAME = "internal"
_BENCHMARK_HIDDEN_USERNAME = "admin@example.com"
...
class Database:
    ...
    def _seed_benchmark_data(self) -> None:
        """Install a hidden row used by the SQL-injection benchmark oracle."""
        self.conn.execute(
            """INSERT OR IGNORE INTO contributors (
                    repo_owner, repo_name, username, platform,
                    trust_tier, latest_score, blocked_reason
               ) VALUES (?, ?, ?, 'github', 'blocked', ?, ?)""",
            [
                _BENCHMARK_HIDDEN_REPO_OWNER,
                _BENCHMARK_HIDDEN_REPO_NAME,
                _BENCHMARK_HIDDEN_USERNAME,
                4.2,
                "benchmark hidden row",
            ],
        )
        self.conn.execute(
            """INSERT OR IGNORE INTO decisions (
                    contributor_id, pr_number, trust_tier, outcome, reason, score_history_id
               ) VALUES (
                    (
    ...
    def get_report_stats(
        ...
        username_filter = username_contains.lower()
        ...
        recent_query = f"""SELECT d.decided_at, c.username, d.trust_tier, d.outcome,
                       d.reason, s.total_score
                FROM decisions d
                JOIN contributors c ON d.contributor_id = c.id
                LEFT JOIN score_history s ON d.score_history_id = s.id
                WHERE {repo_filter} AND c.username LIKE '%{username_filter}%'
                ORDER BY d.decided_at DESC LIMIT 10"""
        result = self.conn.execute(recent_query, repo_params)
